<script>
  ...
  const tints = [
    { num: 'text-pine/40',   dot: 'bg-pine',   text: 'text-pine',   hover: 'hover:border-pine/40',   badge: 'bg-pine/10 text-pine' },
    { num: 'text-ochre/45',  dot: 'bg-ochre',  text: 'text-ochre',  hover: 'hover:border-ochre/40',  badge: 'bg-ochre/10 text-ochre' },
    { num: 'text-accent/35', dot: 'bg-accent', text: 'text-accent', hover: 'hover:border-accent/40', badge: 'bg-accent/10 text-accent' },
  ];
    ...
    <div class="mt-5 grid grid-cols-2 gap-5">
      ...
      >
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          {#if project.status}
            <span class="rounded-full {tint.badge} px-2.5 py-0.5 font-body text-xs font-semibold uppercase tracking-wider">{project.status}</span>
          {/if}
          ...
          <dl class="mt-4 flex flex-wrap gap-x-6 gap-y-2">
            {#each project.metrics as m}
              <div>
                ...
                <dd class="font-body text-[0.7rem] uppercase tracking-wider text-ink-3">{m.label}</dd>
              </div>
            {/each}
          </dl>
        {/if}
        ...
        {#if project.link || project.repo}
          <div class="mt-auto flex flex-wrap gap-5 pt-5">
            {#if project.link}
              <a href={project.link} target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1 font-body text-sm font-semibold {tint.text} transition-opacity hover:opacity-70">
                {$t.projects.visitCta}<span aria-hidden="true">↗</span>
              </a>
            {/if}
            {#if project.repo}
              <a href={project.repo} target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1 font-body text-sm font-semibold text-ink-2 transition-colors hover:text-ink">
  ...
  <div class="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
    {#each categories as cat, i}
      <div class="rounded-2xl border border-line bg-paper-2 p-5">
        <p class="font-body text-sm font-semibold {skillTints[i % skillTints.length]}">{cat.label}</p>
