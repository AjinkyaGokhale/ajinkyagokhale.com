...
<article class="shell pt-12 md:pt-16">
  <a
    href="/blog"
    class="inline-flex items-center gap-1.5 font-body text-sm font-semibold text-ink-2 transition-colors hover:text-accent"
  >
  ...
  <header class="mt-6 max-w-read" use:reveal>
    <div
      class="flex flex-wrap items-center gap-2 font-body text-xs font-semibold uppercase tracking-wider text-ink-3"
    >
      <time datetime={meta.date}>{formatted(meta.date)}</time>
      {#if meta.readingTime}<span>·</span><span>{meta.readingTime} min read</span>{/if}
    </div>
    <h1
      class="mt-3 font-display text-4xl font-semibold leading-[1.08] tracking-tight text-ink sm:text-5xl"
    >
      {meta.title}
    </h1>
